...
export default function SearchPage() {
  ...
  const [results, setResults] = useState([]); // ✅ holds backend data
  ...
  // 🔍 Automatically search while typing (debounced)
  useEffect(() => {
  ...
  const handleSearch = async () => {
    ...
    try {
      let response;
      if (activeTab === "Users") {
        response = await api.get(`/search/name/${encodeURIComponent(query)}`);
      } else if (activeTab === "Projects") {
        response = await api.get(`/search/project/${encodeURIComponent(query)}`);
      } else {
        setResults([]); // for tabs without backend yet
        setLoading(false);
        return;
      }
  ...
  // 👇 if backend returns nothing, fallback to static placeholders for other tabs
        ...
        {loading ? (
          ...
          displayedResults.map((item, index) => (
            ...
            >
              ...
              <p className="text-gray-400 text-sm">
                {activeTab === "Users"
                  ? item.tagline || "Developer on DevConnect"
                  : activeTab === "Projects"
                  ? item.description || "Open-source full-stack project"
                  : activeTab === "Posts"
                  ? "Shared by top contributors"
                  : activeTab === "Tags"
                  ? "Explore projects and posts using this tag"
                  : "Trending topic on DevConnect"}
              </p>
            </motion.div>
          ))
        ) : (
          <p className="text-gray-400 col-span-full text-center mt-10">
            {query
              ...
              : `Start typing to search ${activeTab.toLowerCase()}...`}
          </p>
        )}
      </motion.div>
    </div>
